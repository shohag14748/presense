...
var PxsAnnouncementBar = function () {
  function PxsAnnouncementBar(section) {
    dist_index_es_classCallCheck(this, PxsAnnouncementBar);
  ...
  index_es_createClass(PxsAnnouncementBar, [{
    key: 'onSectionUnload',
    value: function onSectionUnload() {
...
class StaticCart {
  ...
  saveSpecialInstructions() {
    ...
    Shopify.updateCartNote(newNote, cart => {// empty function so that Shopify's callback isn't invoked
    });
  }
  /**
  ...
  _updateCart(thisTimeoutId) {
    shopify_asyncview_dist_index_es.load(window.Theme.routes.cart_url, {
      ...
      Shopify.getCart(cart => jquery_default()('.header-cart-count').text(cart.item_count)); // Inject new cart table contents
...
class Navigation {
  constructor(instance) {
    ...
    jquery_default()('[data-dropdown-toggle]').on('keydown.navigation', event => this.toggleNavigationByKey(event)); // Click
  ...
  setMenuLink($target, e) {
    e.stopPropagation();
    const clickedParent = $target.classList.contains('primary-link') || $target.classList.contains('secondary-link'); // When the dropdown menu is open, redirect parent links
...
class Goal {
  constructor(_ref) {
    ...
    this.$document = jquery_default()(document.body);
    this.context = context; // Product goal can be a dynamic section which uses the global theme settings
    ...
    this.endDateTime = this.$selector.data('end-time'); // TODO: Do better checking or parsing on passed value to make sure its a proper date
...
/* harmony default export */ const ThemeUtils = ({
  ...
  scrollTarget($el) {
    ...
    if ($stickyHeader.length && window.innerWidth >= 11) {
...
class StaticHeader {
  constructor(instance) {
    ...
    this.window.on('resize.header', () => this._resizeEvents()); // This can't be debounced as it creates a visual delay

    this.window.on('scroll.header', () => this._headerOffsets()); // Conditionals

    this.isHeaderNavigation = this.$headerContainer.attr('data-main-header') === 'header';
    this.hasLogo = this.$headerLogo.length;
    this.isHeaderSticky = this.$headerContainer.attr('data-sticky-header') != null; // Imported Classes
  ...
  onSectionUnload() {
    this.$document.off('.header-search-toggle').off('click.search', '[data-search-toggle]').off('blur.search', '.header-search-input').off('click.drawer', '[data-drawer-toggle]').off('calculateHeaderWidths').off('toggleStickyHeader').off('toggleSlideShowHeader').off('checkAnnouncementBar').off('checkHeaderOffsets'); // Remove Goal logic when section unloads
  ...
  _resizeEvents() {
    ...
    const currentWindowWidth = this.themeUtils.windowWidth(); // Only close navigation if width has changed
  ...
  */
  ...
  _headerOffsets($slideShow) {
    ...
    const pastThreshold = pageYOffset > scrollThreshold; // Adjust the header in situations where it is absolute
    ...
    if (this.isHeaderSticky || isFullWindowSlideshow) {
      ...
      if (this.isHeaderSticky) {
        ...
        if (!isFullWindowSlideshow) {
          bodyOffset = scrollThreshold + this.$headerContent.outerHeight(true); // If header is sticky, and no announcement bar, no offset

          if (this.themeUtils.windowWidth() < 11) {
            bodyOffset = 0;
          } // If there is an announcement bar, offset by this its height


          if (this.themeUtils.windowWidth() < 11 && this.announcementBarEl) {
            bodyOffset = this.announcementBarRect.height;
          }
        }
  ...
  _checkAnnouncementBar(data) {
    const {
      el
    } = data; // Refresh globals when AnnouncementBar is updated
...
class StaticSlideshow {
  constructor(_ref) {
    ...
    this._setupSlides(); // Adds helper class to header when slideshow set to 'full window'
  ...
  _setActiveSlideHeight() {
    ...
    if (this.slideshowLayout === 'full-window') {
      const headerHeight = this.headerNavigation.getBoundingClientRect().height; // If there is too much text, the height of the slide should be more than the full
      // height of the window.

      if (textHeight > window.innerHeight - headerHeight) {
        slideHeight = window.innerWidth > 11 ? textHeight + headerHeight * 2 : textHeight;
      } else {
        slideHeight = window.innerWidth > 11 ? this.fullWindowHeight : this.fullWindowHeight - this.headerNavigation.getBoundingClientRect().height;
      }

      const announcementBarEl = document.querySelector('.pxs-announcement-bar'); // If is full window, subtract announcement bar height

      if (announcementBarEl) {
        slideHeight -= announcementBarEl.getBoundingClientRect().height;
      }

      this.slideshowWrapper.style.height = `${slideHeight}px`; // Update height of all slides since they should all be the same height.
  ...
  _showNewSlide(type, specificSlideID) {
    ...
    this.nextSlide.classList.add(type); // force reflow for slide effect
...
class VariantHelper {
  constructor(options) {
    ...
    this.$smartPaymentButtons = this.options.$productForm.find('.shopify-payment-button'); // Disabled history state updating in the TE
...
class Modal {
  ...
  open() {
    this.el.classList.add(this.selectors.open); // Use onOpen to insert contents into modal
...
class PaymentTerms {
  constructor(el) {
    ...
    this._inputs = []; // Launch uses separate markup for desktop and mobile so we need
...
class ProductDetails {
  constructor(section) {
    ...
    this.processing = false; // Form selectors
    ...
    if (quantity === '' || quantity === '0' || isPasswordPage) {
      ...
      .done(cartItem => {
        setTimeout(() => {
          ...
          this.processing = false; // Enable cart redirect if setting enabled
...
const initLaunch = () => {
  ...
  sections.register('pxs-map', section => new pxs_map_dist_index_es(section), {
    lazy: true
  });
...
/******/ })()
;
//# sourceMappingURL=launch.js.map?1650474395236